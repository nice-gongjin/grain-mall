...
public class GlobalAdviceHandler {
    ...
    public R myException(Exception e) {
        return R.error("loginWeb的全局异常处理自定义的MyException异常：" + e.getMessage());
    }

    @ExceptionHandler(Exception.class)
    public R exceptionHandler(Exception e){
        return R.error("loginWeb的全局异常处理的Exception错误：" + e.getMessage());
    }

    @ExceptionHandler(RuntimeException.class)
    public R runtimeExceptionHandler(RuntimeException r){
        return R.error("loginWeb的全局异常处理的RuntimeException错误：" + r.getMessage());
    }
